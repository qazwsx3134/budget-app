...
import { apiUploadSingle,apiUploadMulti,apiUpdateQanda,apiDeleteSingle } from "../api/api";
import { editQanda } from "../actions/qanda";
...
const useStyles = makeStyles(theme => ({
  ...
  setTitle(props.qanda ? props.qanda.title : '')
  setTopic(props.qanda ? props.qanda.topic : '')
  setCity(props.qanda ? props.qanda.city : '')
  setText(props.qanda ? props.qanda.text : '')
  setOtherPic(props.qanda ? props.qanda.otherPic : [])
  ...
  const onSubmit = async(e) =>{
    ...
    const qandadata= {
      ...
      "topic":topic,
      "text":text,
      "author":[{user:props.auth._id,"name": props.auth.firstName}],
      "otherPic":otherPic,
    ...
    if (props.auth.firstName) {
      e.preventDefault();
      const postRes = await apiUpdateQanda(qandadata,props.match.params.id)
        console.log(postRes);
      dispatch(editQanda(props.match.params.id,qandadata))  
        history.goBack()
    }
  }
  return (
